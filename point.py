...
class Point(Item):
    ...
    def move(self, event):
        ...
        #self.y(y)
        if self.x.tag[0] == "X":
            self.canvas.S[self.x.tag] = Formula(str(x))
        elif self.x.tag[0] == "Y":
            self.canvas.S[self.x.tag] = Formula(str(y))
            
        if self.y.tag[0] == "Y":
            self.canvas.S[self.y.tag] = Formula(str(y))
        elif self.y.tag[0] == "X":
            self.canvas.S[self.y.tag] = Formula(str(x))
