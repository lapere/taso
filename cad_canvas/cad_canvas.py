...
class cad_canvas(glcanvas.GLCanvas):
    ...
    def OnMouseLeftDown(self, evt):
        ...
        if self.current:
            if self.e:
                self.e.Cut()
                self.e.WriteText(self.visuals[self.current].tag)
            
            current.mainframe.set_statusbar(self.visuals[self.current].tag)
    ...
    def EvtChar(self, evt):
        ...
        if kc == 27: #esc
            ...
            fun = self.e.GetValue()
            self.current_item.new_formula(fun)
            self.e.Destroy()
            wx.SafeYield()
            self.repaint()
        else:
    ...
    def OnMouseLeftDouble(self, evt):
        if self.current:
            f = self.visuals[self.current]
            self.current_item = f
            self.e = wx.TextCtrl(self, -1, f.formula, size=(200,-1), pos=evt.GetPosition())
            self.e.Bind(wx.EVT_CHAR, self.EvtChar)
    """
            f.new_formula(new_f)
            self.repaint()
