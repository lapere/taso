...
class PLauta(Canvas):
    ...
    def getPoint(self, event=None, rubber=None):
        ...
        if tag[0] == "P":
            ...
            x = Level(self, self.x, "X")
            y = Level(self, self.y, "Y")
            p = self.point(x, y)
            return p

    def find_olap(self, typ):
        t = self.find_overlapping(self.x - 7,
                                      self.y - 7,
                                      self.x + 7,
                                      self.y + 7)
        a = []
    ...
    def e_x(self, event=None):
        if not event:
            ...
            Level(self, self.canvasx(event.x), "X")
            self.unbind("<Button-1>")
    ...
    def e_a(self, event=None):
        if not event:
            self.bind("<Button-1>", self.e_a)
            self.tmp = []
        else:
            self.tmp.append(self.canvasx(event.x))
            self.tmp.append(self.canvasy(event.y))
    ...
    def point(self, x, y):
        ...
        if a:
            for i in a:
                p.new_fellow(self.items[i])
                self.items[i].new_fellow(p)
        self.items[p.tag] = p
        return p
