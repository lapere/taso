...
class VisualBaseItem:
    ...
    def mouse_push(self, event):
        ...
        self.e.icursor(END)
        self.canvas.e['width'] = len(self.cmd.get()) + 1
        self.canvas.update()
        
    def mouse_double(self, event):
        x = self.canvas.canvasx(event.x)
        y = self.canvas.canvasy(event.y)
        self.cmd.set(self.value.formula)
        self.e.bind("<KeyPress>", self.g)
        if not self.id_txt:
            self.id_txt = self.canvas.create_window(x, y, window=self.e)
        else:
            self.canvas.e = self.e
            self.canvas.cmd = self.cmd
            self.canvas.coords(self.id_txt, x, y)

    def g(self, event):
        self.e['width'] = len(self.cmd.get()) + 1
        if event.keysym == "Return":
            self.value.new_formula(self.cmd.get())
            ...
            self.canvas.e = self.canvas.e_org
            self.canvas.cmd = self.canvas.cmd_org
            self.canvas.focus_set()

    def entry(self, on):
        if on:
            self.cmd = StringVar(self.canvas)
            self.e = Entry(self.canvas, width=3, bg="white", relief=FLAT, bd=0, textvariable=self.cmd)
            self._e = True
        else:
            self.e = self.canvas.e
            self.cmd = self.canvas.cmd
            self._e = False
...
class VisualItem(VisualBaseItem, Item):
