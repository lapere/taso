...
class PLauta(Canvas):
    ...
    def noMore(self, event):
        ...
        self._more = False
        if self.currenttool:
            if len(self.tmp.fellows) == 0:
                self.items.pop(self.tmp.tag)
    ...
    def xelement(self):
        ...
        self._more = True
        self.tmp = e
        self.currenttool = "xelement"

    def yelement(self):
        
        if self.currenttool == "yelement":
            if len(self.tmp.fellows) == 0:
                self.items.pop(self.tmp.tag)
